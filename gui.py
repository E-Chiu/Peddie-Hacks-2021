...
encryptColumn = [
    ...
    [sg.Combo([i for i in listdir("./profiles/") if isfile(join("./profiles/", i))], size=(30,1), key="_PROFILES_", enable_events=True), sg.Button("Remove Current Profile", key="_REMOVE-PROF_", size=(20,1))], # choose profile
    [sg.In(size=(70,1), key="_FILES_"), sg.FilesBrowse("Add Files", target="_FILES_")], # browse files to encrypt
...
while True:
    ...
    if event == "_NEW-PROF_":
        crypt.add_Profile(fernet, identifier, values["_NEW-PROF-IN_"]) # creates a profile
        window["_PROFILES_"].update(values=[i for i in listdir("./profiles/") if isfile(join("./profiles/", i))])
